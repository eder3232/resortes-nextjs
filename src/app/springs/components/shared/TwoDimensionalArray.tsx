...
const TwoDimensionalArray = ({ arr, name, decimals = 2 }: Props) => {
  ...
  return (
    <div className='my-4'>
      <table className='table w-auto table-auto bg-neutral overflow-hidden'>
        <thead>
          ...
          <tr className='bg-primary'>
                        ...
                        decimalsArray.map((e, index) => (
                          ...
                          >
                            {({ selected }) => (
                              <>
                                <span
                                  className={`w-8 block truncate ${selected ? 'font-medium' : 'font-normal'
                                    }`}
                                >
                                  {e.name}
                                </span>
            ...
            {
              arr[0].map((e, index) => (
                <th className='bg-neutral-focus text-center text-base text-neutral-content font-bold' key={index}>{index + 1}</th>
              ))
            }
          </tr>
        </thead>
        <tbody>
          {
            arr.map((row, index) => (
              <tr key={index}>
                <td className="bg-neutral-focus text-center text-base text-neutral-content font-bold">{index + 1}</td>
                {
                  row.map((col, index) => (
                    <td key={index} className='text-center text-base w-16'>{typeof col === 'number' ? col.toFixed(selected.id) : col}</td>
                  ))
                }
              </tr>
            ))
